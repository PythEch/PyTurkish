...
"""
    ...
    __sayilar={2: {'1':'on', '2':'yirmi', '3':'otuz', 
                      '4':u'kırk', '5':'elli', '6':u'altmış',
                      '7':'yetmiş', '8':'seksen', '9':'doksan'},
               1: {'0':u'sıfır', '1':'bir', '2':'iki', '3':u'üç',
                       '4':u'dört', '5':u'beş', '6':u'altı', '7':'yedi', 
                       '8':'sekiz', '9':'dokuz'}}
    #Tamamlanacak
    _istisna={u'kök':{u'şu':u'şun', 'bu':'bun', 'o':'on'},#Ek aldıklarında kökü değişen sözcükler
              u'sözcük':{'bene':'bana', 'sene':'sana'}, #Diğer istisnalar
              u'ünlü':{'renk':'reng', 'cenk':'ceng', 'his':'hiss', 'isim':'ism', 'beyin':'beyn', 'boyun':'boyn', u'karın':'karn', 'af':'aff',
                    u'kısım':u'kısm', 'nesil':'nesl', 'burun':'burn', u'zulüm':'zulm', u'akıl':'akl', u'asıl':'asl' ,u'asır':'asr', 'devir':'devr', 
                    'emir':'emr', 'fikir':'fikr', 'ilim':'ilm', u'kayıt':'kayd', u'keşif':u'keşf', 'keyif':'keyf', 'nakil':'nakl', 'nehir':'nehr',
                    u'sabır':'sabr', 'seyir':'seyr', u'şehir':u'şehr', u'şekil':u'şekl', 'zehir':'zehr', 'zihin':'zihn', 'zan':'zann',u'bağır':u'bağr',
                    u'ağız':u'ağz', u'alın':'aln', 'beniz':'benz', u'böğür':u'böğr', 'geniz':'genz', u'göğüs':u'göğs', u'gönül':u'gönl', u'oğul':u'oğl',
                    'resim':'resm', u'kayın':'kayn', u'kayıp':'kayb', u'nabız':'nabz', 'ret':'redd', 'kalp':'kalb'}, #Sadece ünlü ek aldıklarında kökü değişen sözcükler (ünlü düşmesi)
              #Büyük ünlü uyumu istisnaları #Cem Yıldız'a teşekkürler
              u'büu': ('kontrol', 'bandrol', 'banal', 'alpul', 'ametal', 'anormal', 'amiral'
                       , 'sadakat', 'santral', u'şefkat', 'usul', 'normal', 'oryantal', 'hakikat'
                       , 'hayal', 'saat', 'kemal', 'gol', 'kalb', 'metal', 'faul', 'mineral', 'alkol'
                       , 'misal', 'meal', 'oramiral', u'tuğamiral', 'orjinal','orijinal', 'koramiral', 'general'
                       , u'tümgeneral', u'tuğgeneral', 'korgeneral', 'petrol', 'liberal', 'meral'
                       , 'metropol', u'ekümenapol', 'lokal', u'lügat', 'liyakat', 'legal', 'mentol'
                       , u'beşamol', u'meşgul', u'meşekkat', 'oval', 'mahsul', 'makul', 'meraşal'
                       ...
                       , 'kolestrol', u'kıraat', 'ziraat', 'kapital', 'katedral', 'kabul', 'kanaat', 'jurnal'
...
Örnek: ek('Kapı').nin()+' '+ek('kol').i()"""
    ...
    def __isle__(self):
        ...
        #Ünsüz Yumuşaması
        if not self.__ozelIsim and self._kacHeceli() > 1 and self.__sozcuk.endswith(tuple(self.__yumusama.keys())) and self.__ek.startswith(self.__unluler[u'tüm']):
                k=self.__yumusama[self.__sozcuk[-1]]
                if self.__sozcuk[-2] == 'n' and k==u'ğ':
                    self.__sozcuk = self.__sozcuk[:-1]+'g'
                elif k!=u'ğ' or self.__sozcuk[-2] in self.__unluler[u'tüm']:
        ...
        if not self._inceMi() and not self.__sozcuk in self._istisna[u'büu']:
            ...
            for i in self.__unluler['ince']:
                self.__ek=self.__ek.replace(i,self.__unluler[u'kalın'][n])
                n+=1
        #Kaynaştırma Ünsüzü
        ...
        #İstisna 3
        if self.__sozcuk in self._istisna[u'sözcük']:
            self.__sozcuk = self._istisna[u'sözcük'][self.__sozcuk]
        #Return
        if self.__asilSozcuk.isupper():
            ...
            self.__sozcuk = self.__sozcuk[0].replace('i',u'İ').replace(u'ı','I') + self.__sozcuk[1:] #Python Türkçe bug-fix
